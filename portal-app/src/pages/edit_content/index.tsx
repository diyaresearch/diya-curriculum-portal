...
import type { UnitDocument } from "@/types/models";
import "react-quill-new/dist/quill.snow.css"; // Import Quill CSS

/**
 * Every field is a string because they are all bound to controlled inputs.
 * The stored document allows `Duration` to be a number (see UnitDocument),
 * so it is stringified on read and sent back as typed.
 */
...
export const EditContent = () => {
  ...
  useEffect(() => {
    ...
    const fetchContent = async () => {
      ...
      try {
        const data = await api.get<UnitDocument>(`/api/unit/${contentId}`, { auth: false });
        if (cancelled || !data) return;
        // Every field is optional in the stored document, and feeding
        // `undefined` to a controlled input makes React switch it to
        // uncontrolled halfway through the form's life.
        setFormData({
          Title: data.Title ?? "",
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setModalMessage(
        "Error updating content:" + (error instanceof Error ? error.message : String(error))
      );
      setModalIsOpen(true);
