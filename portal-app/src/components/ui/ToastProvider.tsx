...
export interface ToastOptions {
  tone?: ToastTone;
  /** Milliseconds before auto-dismiss; 0 or less keeps it up. */
  duration?: number;
}
...
/**
 * `message` is ReactNode and may be null: toUserMessage() returns null for a
 * cancelled request, and every raiser is allowed to pass that straight
 * through rather than guarding it.
 */
type ShowToast = (message: ReactNode, options?: ToastOptions) => number | null;
...
// null is the "no provider above me" sentinel that useToast() throws on.
...
const TONE: Record<ToastTone, { skin: string; icon: string }> = {
...
export function ToastProvider({ children }: { children?: ReactNode }) {
  ...
  const show = useCallback<ShowToast>(
    (message, { tone = "info", duration = AUTO_DISMISS_MS }: ToastOptions = {}) => {
