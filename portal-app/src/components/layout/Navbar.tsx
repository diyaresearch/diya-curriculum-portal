...
/** The fields the (currently unreachable) signup popup collects. */
interface SignUpFormData {
  ...
  userType: string;
  jobTitle: string;
  /** One subject, chosen from a single-select. See handleSignUpSubmit. */
  subjects: string;
}
      ...
      await startGoogleRedirect({
        ...
        action: {
          ...
          payload: {
            ...
            // The form picks one subject, but users.subjects is a list and
            // POST /api/user/register stores `Array.isArray(subjects) ?
            // subjects : []` - so sending the bare string silently saved an
            // empty list. Typing the payload against UserDocument is what
            // turned that up.
