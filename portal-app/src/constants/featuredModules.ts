...
/** One row of the "Category / Level / Type / Duration" table on a module page. */
export interface FeaturedModuleDetail {
  label: "Category" | "Level" | "Type" | "Duration";
  value: string;
}

/**
 * An item in a featured module's resource list. It has no `id`: these
 * modules are not Firestore documents, so the lesson view reached from here
 * routes by /lesson/:moduleId/:lessonIndex rather than by a lesson id.
 */
export interface FeaturedModuleResource {
  title: string;
  desc: string;
  type: "Lesson Plan" | "Assignment" | "Project" | "Lecture" | "Discussion";
  /** Minutes. Present only on the modules whose resources are timed. */
  duration?: number;
  locked: boolean;
}
...
export interface FeaturedModule {
  title: string;
  subtitle: string;
  /** Imported PNG; Vite resolves it to a URL string. */
  image: string;
  description: string;
  requirements: string;
  learningObjectives: string;
  details: FeaturedModuleDetail[];
  resources: FeaturedModuleResource[];
}
 ...
 *
 * Typed as a lookup that can miss: every caller indexes it with a :moduleId
 * straight off the URL, so `FEATURED_MODULES[id]` has to be
 * `FeaturedModule | undefined` for the existing `|| null` guards to be the
 * ones the compiler asks for.
 */
export const FEATURED_MODULES: Record<string, FeaturedModule | undefined> = {
  "python-for-ai": {
    title: "PYTHON FOR AI",
